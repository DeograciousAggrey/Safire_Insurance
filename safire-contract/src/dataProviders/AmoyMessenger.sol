...
contract AMOYMessenger is CCIPReceiver, OwnerIsCreator {
    ...
    constructor(address _router, address _link, uint256 _sourceSelector) CCIPReceiver(_router) {
    ...
    function _ccipReceive(
        ...
        if (keccak256(abi.encodePacked(lastReceivedText)) == keccak256(abi.encodePacked("check_lending_status"))) {
    ...
    function sendMessageBackPayLINK(address _receiver, string memory _text, uint64 _sourceSelector)
        internal
        returns (bytes32 messageId)
    {
        ...
        emit MessageSent(messageId, _sourceSelector, _receiver, _text, address(s_linkToken), fees);
